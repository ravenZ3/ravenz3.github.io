...
export const COLLECTIONS = ['literary', 'reflections', 'finance', 'papers', 'projects'] as const;
...
export async function getAllNotes() {
  const [literary, reflections, finance, papers, projects] = await Promise.all([
...
//   [text](/literary/slug)  [text](/reflections/slug)  [text](/papers/slug)  [text](/projects/slug)
// Returns the slugs of all linked notes.
function extractInternalLinks(body: string): string[] {
  const pattern = /\[([^\]]+)\]\(\/(literary|reflections|finance|papers|projects)\/([^)#?\s/]+)\)/g;
  const slugs: string[] = [];
